...
import { RoomDataProvider, useRoomData } from '@/lib/RoomDataContext'
...
function RoomLayoutInner({ code, children }: { code: string; children: React.ReactNode }) {
  const { feed: { loading: feedLoading }, poop: { loading: poopLoading } } = useRoomData()
  ...
  useEffect(() => {
    if (dataReady && !fadeStarted.current) {
...
export default function RoomLayout({ children, params }: RoomLayoutProps) {
  return (
    <RoomDataProvider code={params.code}>
      <RoomLayoutInner code={params.code}>
        {children}
      </RoomLayoutInner>
    </RoomDataProvider>
  )
